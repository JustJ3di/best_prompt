...
# UI con Streamlit
st.title("Classificazione Prompt con LLM")
st.write("Inserisci un prompt per ottenere il punteggio e la similarità rispetto ai dati esistenti.")
...
new_prompt = st.text_area("Inserisci il tuo prompt:")
if st.button("Classifica"):
    if new_prompt:
        score, similarity = classify_prompt(new_prompt, model, embeddings, prompts)
        st.metric(label="Punteggio Assegnato", value=round(score, 2))
        st.metric(label="Similarità", value=f"{similarity:.2f}")
    else:
